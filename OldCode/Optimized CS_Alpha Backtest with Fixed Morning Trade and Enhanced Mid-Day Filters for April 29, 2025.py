...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    morning_done  = False
    atr_mean      = df['ATR_10'].rolling(window=10, min_periods=1).mean()
    atr_std       = df['ATR_10'].rolling(window=10, min_periods=1).std()

    # Market condition
    df['Market_Condition'] = 'ranging'
    df.loc[df['ADX'] > 30, 'Market_Condition'] = 'trending'
    df.loc[df['ATR_10'] > (atr_mean + 0.2 * atr_std), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                sl_mult = 2.0 if r['Market_Condition'] == 'volatile' else 2.5 if r['Market_Condition'] == 'trending' else 3.0
            ...
            flip = (side == 'buy' and sell_score >= 4.0) or (side == 'sell' and buy_score >= 4.0)
            rsi_exit = r['RSI_10'] > 85 or r['RSI_10'] < 15
            max_hold = ((bar_count >= 10 and r['Market_Condition'] == 'ranging') or
                       (bar_count >= 15 and r['Market_Condition'] == 'trending'))
        ...
        if (buy_score >= 3.5 and r['ATR_10'] > 12 and df['SuperTrend_dir'].iloc[i] == 1 and
            abs(r['close'] - p['close']) > r['ATR_10'] * 0.5):
            entry = r['open']
            sl_mult = 2.0 if r['Market_Condition'] == 'volatile' else 2.5 if r['Market_Condition'] == 'trending' else 3.0
            stop  = entry - r['ATR_10'] * sl_mult * 2.0
        ...
        if (sell_score >= 4.5 and r['ATR_10'] > 12 and df['SuperTrend_dir'].iloc[i] == -1 and
            abs(r['close'] - p['close']) > r['ATR_10'] * 0.5):
            entry = r['open']
            sl_mult = 2.0 if r['Market_Condition'] == 'volatile' else 2.5 if r['Market_Condition'] == 'trending' else 3.0
            stop  = entry + r['ATR_10'] * sl_mult * 2.0
