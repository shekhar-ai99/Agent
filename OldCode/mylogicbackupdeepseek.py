...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('signals_enhanced.log'),
...
DEFAULT_OUTPUT_FILE = DATA_FOLDER / "nifty_signals_final_enhanced.csv"
DEFAULT_PLOT_FILE = DATA_FOLDER / "signals_plot_enhanced.png"
...
class TradeState:
    ...
    def update_trailing_stop(self, current_low, current_high, current_close, atr):
        if self.position is None or atr is None or pd.isna(atr):
            return
        ...
        if self.position == 'Long':
            ...
            if stop_type == "percentage":
                potential_stop = self.highest_high_in_trade * (1 - stop_pct)
            else:
                potential_stop = self.highest_high_in_trade - (atr * current_atr_mult)

            if self.trailing_stop is None or potential_stop > self.trailing_stop:
            ...
            if stop_type == "percentage":
                potential_stop = self.lowest_low_in_trade * (1 + stop_pct)
            else:
                potential_stop = self.lowest_low_in_trade + (atr * current_atr_mult)
...
def calculate_fibonacci_levels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df['rolling_high'] = df['high'].rolling(window=2*lookback+1, center=True).max()
        df['rolling_low'] = df['low'].rolling(window=2*lookback+1, center=True).min()
        ...
        df['last_pivot_high_idx_num'] = df['last_pivot_high_idx'].apply(
            lambda x: df.index.get_loc(x) if pd.notna(x) else np.nan)
        df['last_pivot_low_idx_num'] = df['last_pivot_low_idx'].apply(
            lambda x: df.index.get_loc(x) if pd.notna(x) else np.nan)
        ...
        swing_high = np.where(df['last_pivot_high_idx_num'] > df['last_pivot_low_idx_num'], 
                             df['last_pivot_high_price'], df['last_pivot_low_price'])
        swing_low = np.where(df['last_pivot_high_idx_num'] > df['last_pivot_low_idx_num'],
                           df['last_pivot_low_price'], df['last_pivot_high_price'])
        is_uptrend_fib = (df['last_pivot_high_idx_num'] > df['last_pivot_low_idx_num']).fillna(False)
        ...
        df.drop(columns=[
            'rolling_high', 'rolling_low', 'is_pivot_high', 'is_pivot_low',
            'last_pivot_high_price', 'last_pivot_high_idx', 'last_pivot_low_price',
            'last_pivot_low_idx', 'last_pivot_high_idx_num', 'last_pivot_low_idx_num'
...
def calculate_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Initialize all condition columns with False
        condition_columns = [
            'cond_ema_trend_buy', 'cond_ema_trend_sell',
            'cond_ema_signal_buy', 'cond_ema_signal_sell',
            'cond_rsi_buy', 'cond_rsi_sell',
            'cond_macd_signal_buy', 'cond_macd_signal_sell',
            'cond_macd_zero_buy', 'cond_macd_zero_sell',
            'cond_vol_break_buy', 'cond_vol_break_sell',
            'cond_adx_strength', 'cond_adx_dir_buy', 'cond_adx_dir_sell',
            'cond_fib_bounce_buy', 'cond_fib_bounce_sell',
            'cond_ema_bounce_buy', 'cond_ema_bounce_sell',
            'cond_bb_bounce_buy', 'cond_bb_bounce_sell'
        ]
        ...
        if PARAMS['trend']['use_ema_trend_filter']:
            buy_score += df['cond_ema_trend_buy'] * weights.get('w_ema_trend', 0)
            sell_score += df['cond_ema_trend_sell'] * weights.get('w_ema_trend', 0)
        
        buy_score += df['cond_ema_signal_buy'] * weights.get('w_ema_signal', 0)
        sell_score += df['cond_ema_signal_sell'] * weights.get('w_ema_signal', 0)
        buy_score += df['cond_rsi_buy'] * weights.get('w_rsi_thresh', 0)
        sell_score += df['cond_rsi_sell'] * weights.get('w_rsi_thresh', 0)
        buy_score += df['cond_macd_signal_buy'] * weights.get('w_macd_signal', 0)
        sell_score += df['cond_macd_signal_sell'] * weights.get('w_macd_signal', 0)
        buy_score += df['cond_macd_zero_buy'] * weights.get('w_macd_zero', 0)
        sell_score += df['cond_macd_zero_sell'] * weights.get('w_macd_zero', 0)
        
        if PARAMS['core']['use_vol_breakout_buy'] or PARAMS['core']['use_vol_breakout_sell']:
            buy_score += df['cond_vol_break_buy'] * weights.get('w_vol_break', 0)
            sell_score += df['cond_vol_break_sell'] * weights.get('w_vol_break', 0)
        
        if PARAMS['trend']['use_adx_filter']:
            buy_score += (df['cond_adx_strength'] & df['cond_adx_dir_buy']) * weights.get('w_adx_strength', 0)
            sell_score += (df['cond_adx_strength'] & df['cond_adx_dir_sell']) * weights.get('w_adx_strength', 0)
        
        if PARAMS['trend']['use_adx_direction_filter']:
            buy_score += df['cond_adx_dir_buy'] * weights.get('w_adx_direction', 0)
            sell_score += df['cond_adx_dir_sell'] * weights.get('w_adx_direction', 0)

        df['buy_score_raw'] = buy_score
        df['sell_score_raw'] = sell_score
        df['scaled_score'] = 5.0  # Default neutral score
...
def calculate_targets_for_trade(df: pd.DataFrame, entry_idx: int, position: str, entry_price: float, initial_stop: float) -> pd.DataFrame:
    try:
        ...
        atr_mult_target = PARAMS['atr'].get('atr_mult', 2.0)
...
def process_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if PARAMS['trend']['use_adx_filter'] or PARAMS['trend']['use_adx_direction_filter']:
            ...
            df['plus_di'] = 100 * (smooth_plus_dm / tr_smoothed)
            df['minus_di'] = 100 * (smooth_minus_dm / tr_smoothed)
        ...
        df['exit_reason'] = ''
        df['entry_signal_type'] = ''
        df['target_price_long'] = np.nan
        df['target_price_short'] = np.nan
        df['fib_target_long'] = np.nan
        df['fib_target_short'] = np.nan
        ...
        start_index = max(
            ...
            PARAMS['atr']['atr_len'],
            (PARAMS['trend']['adx_len'] * 2) if (PARAMS['trend']['use_adx_filter'] or PARAMS['trend']['use_adx_direction_filter']) else 0,
        ...
        weights = PARAMS['score_weights']
        total_possible_score = max(1, sum(weights.values()))
        ...
        for i in range(start_index, len(df)):
            ...
            if trade_state.position is not None:
                ...
                if PARAMS['core']['trailing_stop_type'] != "none" and trade_state.trailing_stop is not None:
                    if trade_state.position == 'Long' and current['low'] <= trade_state.trailing_stop:
                        exit_reason = 'Trailing Stop'
                        exit_price_this_bar = min(current['open'], trade_state.trailing_stop)
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and current['high'] >= trade_state.trailing_stop:
                        exit_reason = 'Trailing Stop'
                        exit_price_this_bar = max(current['open'], trade_state.trailing_stop)
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and PARAMS['rsi']['rsi_len'] > 0:
                    if trade_state.position == 'Long' and current['bearish_rsi_div']:
                        exit_reason = 'RSI Div'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and current['bullish_rsi_div']:
                        exit_reason = 'RSI Div'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and PARAMS['fibonacci']['use_fib_exit']:
                    fib_target_long = df.loc[current_idx, 'fib_target_long']
                    fib_target_short = df.loc[current_idx, 'fib_target_short']
                    if trade_state.position == 'Long' and pd.notna(fib_target_long) and current['high'] >= fib_target_long:
                        exit_reason = 'Fib Target'
                        exit_price_this_bar = max(current['open'], fib_target_long)
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and pd.notna(fib_target_short) and current['low'] <= fib_target_short:
                        exit_reason = 'Fib Target'
                        exit_price_this_bar = min(current['open'], fib_target_short)
                        exit_triggered_this_bar = True
                
                # Recalculate score for exit check
                buy_score_bar_exit = df.loc[current_idx, 'buy_score_raw']
                sell_score_bar_exit = df.loc[current_idx, 'sell_score_raw']
                if df.loc[current_idx, 'cond_fib_bounce_buy']:
                    buy_score_bar_exit += weights.get('w_fib_bounce', 0)
                if df.loc[current_idx, 'cond_fib_bounce_sell']:
                    sell_score_bar_exit += weights.get('w_fib_bounce', 0)
                if df.loc[current_idx, 'cond_ema_bounce_buy']:
                    buy_score_bar_exit += weights.get('w_ema_bounce', 0)
                if df.loc[current_idx, 'cond_ema_bounce_sell']:
                    sell_score_bar_exit += weights.get('w_ema_bounce', 0)
                if df.loc[current_idx, 'cond_bb_bounce_buy']:
                    buy_score_bar_exit += weights.get('w_bb_bounce', 0)
                if df.loc[current_idx, 'cond_bb_bounce_sell']:
                    sell_score_bar_exit += weights.get('w_bb_bounce', 0)
                ...
                df.loc[current_idx, 'scaled_score'] = scaled_score_bar_exit
                
                if not exit_triggered_this_bar and PARAMS['core']['use_score_drop_exit']:
                    score_threshold = PARAMS['core']['exit_score_drop_threshold']
                    if trade_state.position == 'Long' and scaled_score_bar_exit < (5.0 - score_threshold):
                        exit_reason = f'Score Drop ({scaled_score_bar_exit:.1f})'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and scaled_score_bar_exit > (5.0 + score_threshold):
                        exit_reason = f'Score Drop ({scaled_score_bar_exit:.1f})'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and PARAMS['ema']['fast_len'] > 0:
                    fast_ema_cross_med_sell = current['ema_fast'] < current['ema_med'] and prev['ema_fast'] >= prev['ema_med']
                    fast_ema_cross_med_buy = current['ema_fast'] > current['ema_med'] and prev['ema_fast'] <= prev['ema_med']
                    if trade_state.position == 'Long' and fast_ema_cross_med_sell:
                        exit_reason = 'EMA Cross'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and fast_ema_cross_med_buy:
                        exit_reason = 'EMA Cross'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and PARAMS['bollinger']['bb_len'] > 0:
                    cross_under_bb_mid = current['close'] < current['bb_middle'] and prev['close'] >= prev['bb_middle']
                    cross_over_bb_mid = current['close'] > current['bb_middle'] and prev['close'] <= prev['bb_middle']
                    if trade_state.position == 'Long' and cross_under_bb_mid:
                        exit_reason = 'BB Mid Exit'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and cross_over_bb_mid:
                        exit_reason = 'BB Mid Exit'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and PARAMS['volume']['vol_ma_len'] > 0:
                    low_vol = current['volume'] < df.loc[prev_idx, 'vol_ma']
                    pullback_long = current['close'] < current['ema_fast']
                    pullback_short = current['close'] > current['ema_fast']
                    if trade_state.position == 'Long' and low_vol and pullback_long:
                        exit_reason = 'Vol Fade'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                    elif trade_state.position == 'Short' and low_vol and pullback_short:
                        exit_reason = 'Vol Fade'
                        exit_price_this_bar = potential_exit_price
                        exit_triggered_this_bar = True
                
                if not exit_triggered_this_bar and trade_state.time_in_trade >= PARAMS['core']['max_trade_duration']:
            ...
            if trade_state.position is None and not exit_triggered_this_bar:
                entry_signal = None
                buy_score_bar = df.loc[current_idx, 'buy_score_raw']
                sell_score_bar = df.loc[current_idx, 'sell_score_raw']
                df.loc[current_idx, ['cond_fib_bounce_buy', 'cond_fib_bounce_sell',
                                   'cond_ema_bounce_buy', 'cond_ema_bounce_sell',
                                   'cond_bb_bounce_buy', 'cond_bb_bounce_sell']] = False

                # Calculate bounce/breakout conditions & update score
                fib_bounce_buy_cond = False
                fib_bounce_sell_cond = False
                if PARAMS['core']['use_fib_bounce_entry'] or PARAMS['core']['use_fib_bounce_sell']:
                    lookback = PARAMS['core']['fib_bounce_lookback']
                    low_zone_long, high_zone_long = PARAMS['core']['fib_bounce_long_zone']
                    low_zone_short, high_zone_short = PARAMS['core']['fib_bounce_short_zone']
                    conf_level = PARAMS['core']['fib_bounce_confirmation_level']
                    ...
                    if PARAMS['core']['use_fib_bounce_entry'] and current['is_uptrend_fib']:
                        fib_low_name = f'fib_{int(low_zone_long*1000)}'
                        fib_high_name = f'fib_{int(high_zone_long*1000)}'
                        fib_conf_name = f'fib_{int(conf_level*1000)}'
                        if fib_low_name in df.columns and fib_high_name in df.columns and fib_conf_name in df.columns:
                            fib_low = current[fib_low_name]
                            fib_high = current[fib_high_name]
                            fib_conf = current[fib_conf_name]
                            if pd.notna(fib_low) and pd.notna(fib_high) and pd.notna(fib_conf):
                                touched_zone = (df['low'].iloc[i-lookback:i] <= fib_high).any()
                                bounced_above = current['close'] > fib_conf
                                rsi_ok = not rsi_conf or (current['rsi'] > rsi_buy_lvl and current['rsi'] > prev['rsi'])
                                if touched_zone and bounced_above and rsi_ok:
                                    fib_bounce_buy_cond = True
                                    df.loc[current_idx, 'cond_fib_bounce_buy'] = True
                                    buy_score_bar += weights.get('w_fib_bounce', 0)
                    
                    if PARAMS['core']['use_fib_bounce_sell'] and not current['is_uptrend_fib']:
                        fib_low_name = f'fib_{int(low_zone_short*1000)}'
                        fib_high_name = f'fib_{int(high_zone_short*1000)}'
                        fib_conf_name = f'fib_{int(conf_level*1000)}'
                        if fib_low_name in df.columns and fib_high_name in df.columns and fib_conf_name in df.columns:
                            fib_low = current[fib_low_name]
                            fib_high = current[fib_high_name]
                            fib_conf = current[fib_conf_name]
                            if pd.notna(fib_low) and pd.notna(fib_high) and pd.notna(fib_conf):
                                touched_zone = (df['high'].iloc[i-lookback:i] >= fib_low).any()
                                bounced_below = current['close'] < fib_conf
                                rsi_ok = not rsi_conf or (current['rsi'] < rsi_sell_lvl and current['rsi'] < prev['rsi'])
                                if touched_zone and bounced_below and rsi_ok:
                                    fib_bounce_sell_cond = True
                                    df.loc[current_idx, 'cond_fib_bounce_sell'] = True
                                    sell_score_bar += weights.get('w_fib_bounce', 0)
                
                ema_bounce_buy_cond = False
                ema_bounce_sell_cond = False
                if PARAMS['core']['use_ema_bounce_buy'] or PARAMS['core']['use_ema_bounce_sell']:
                    lookback = PARAMS['core']['ema_bounce_lookback']
                    ema_source_col = 'ema_fast' if PARAMS['core']['ema_bounce_source_str'] == "Fast EMA" else 'ema_med'
                    ema_source = df[ema_source_col]
                    rsi_conf = PARAMS['rsi']['rsi_confirm_ema']
                    rsi_buy_lvl, rsi_sell_lvl = PARAMS['rsi']['rsi_confirm_level_buy'], PARAMS['rsi']['rsi_confirm_level_sell']
                    
                    if PARAMS['core']['use_ema_bounce_buy']:
                        touched_ema = (df['low'].iloc[i-lookback:i] <= ema_source.iloc[i-lookback:i]).any()
                        bounced_above = current['close'] > current[ema_source_col] and current['close'] > current['open']
                        rsi_ok = not rsi_conf or (current['rsi'] > rsi_buy_lvl and current['rsi'] > prev['rsi'])
                        if touched_ema and bounced_above and rsi_ok:
                            ema_bounce_buy_cond = True
                            df.loc[current_idx, 'cond_ema_bounce_buy'] = True
                            buy_score_bar += weights.get('w_ema_bounce', 0)
                    
                    if PARAMS['core']['use_ema_bounce_sell']:
                        touched_ema = (df['high'].iloc[i-lookback:i] >= ema_source.iloc[i-lookback:i]).any()
                        bounced_below = current['close'] < current[ema_source_col] and current['close'] < current['open']
                        rsi_ok = not rsi_conf or (current['rsi'] < rsi_sell_lvl and current['rsi'] < prev['rsi'])
                        if touched_ema and bounced_below and rsi_ok:
                            ema_bounce_sell_cond = True
                            df.loc[current_idx, 'cond_ema_bounce_sell'] = True
                            sell_score_bar += weights.get('w_ema_bounce', 0)
                
                bb_bounce_buy_cond = False
                bb_bounce_sell_cond = False
                if PARAMS['core']['use_bb_mid_bounce_buy'] or PARAMS['core']['use_bb_mid_bounce_sell']:
                    lookback = PARAMS['core']['bb_bounce_lookback']
                    bb_mid = df['bb_middle']
                    rsi_conf = PARAMS['rsi']['rsi_confirm_bb']
                    rsi_buy_lvl, rsi_sell_lvl = PARAMS['rsi']['rsi_confirm_level_buy'], PARAMS['rsi']['rsi_confirm_level_sell']
                    
                    if PARAMS['core']['use_bb_mid_bounce_buy']:
                        touched_bb = (df['low'].iloc[i-lookback:i] <= bb_mid.iloc[i-lookback:i]).any()
                        bounced_above = current['close'] > current['bb_middle'] and current['close'] > current['open']
                        rsi_ok = not rsi_conf or (current['rsi'] > rsi_buy_lvl and current['rsi'] > prev['rsi'])
                        if touched_bb and bounced_above and rsi_ok:
                            bb_bounce_buy_cond = True
                            df.loc[current_idx, 'cond_bb_bounce_buy'] = True
                            buy_score_bar += weights.get('w_bb_bounce', 0)
                    
                    if PARAMS['core']['use_bb_mid_bounce_sell']:
                        touched_bb = (df['high'].iloc[i-lookback:i] >= bb_mid.iloc[i-lookback:i]).any()
                        bounced_below = current['close'] < current['bb_middle'] and current['close'] < current['open']
                        rsi_ok = not rsi_conf or (current['rsi'] < rsi_sell_lvl and current['rsi'] < prev['rsi'])
                        if touched_bb and bounced_below and rsi_ok:
                            bb_bounce_sell_cond = True
                            df.loc[current_idx, 'cond_bb_bounce_sell'] = True
                            sell_score_bar += weights.get('w_bb_bounce', 0)
                
                vol_breakout_buy_cond = False
                vol_breakout_sell_cond = False
                if PARAMS['core']['use_vol_breakout_buy'] or PARAMS['core']['use_vol_breakout_sell']:
                    high_vol = current['volume'] > current['vol_ma'] * PARAMS['volume']['vol_multiplier']
                    lookback_pa = 5
                    if i >= lookback_pa:
                        recent_high = df['high'].iloc[i-lookback_pa:i].max()
                        recent_low = df['low'].iloc[i-lookback_pa:i].min()
                    else:
                        recent_high = np.nan
                        recent_low = np.nan
                    
                    if pd.notna(recent_high) and PARAMS['core']['use_vol_breakout_buy'] and high_vol and current['close'] > current['open'] and current['close'] > recent_high:
                        vol_breakout_buy_cond = True
                    
                    if pd.notna(recent_low) and PARAMS['core']['use_vol_breakout_sell'] and high_vol and current['close'] < current['open'] and current['close'] < recent_low:
                        vol_breakout_sell_cond = True

                # Final score calculation for the bar
                net_score_bar = buy_score_bar - sell_score_bar
                
                # Calculate current_total_possible more robustly
                current_total_possible = 0
                for k, w in weights.items():
                    parts = k.split('_')
                    if len(parts) >= 3:  # Ensure we have at least 3 parts (w_ema_trend)
                        condition_col = f'cond_{parts[1]}_{parts[-1]}'
                        if condition_col in df.columns:
                            if df.loc[current_idx, condition_col] or 'bounce' not in k:
                                current_total_possible += w
                
                safe_total_score = max(1, current_total_possible if current_total_possible > 0 else total_possible_score)
                ...
                trend_ok_sell = True
                if PARAMS['trend']['use_ema_trend_filter']:
                    trend_ok_buy &= current['cond_ema_trend_buy']
                    trend_ok_sell &= current['cond_ema_trend_sell']
                if PARAMS['trend']['use_adx_filter']:
                    trend_ok_buy &= current['cond_adx_strength']
                    trend_ok_sell &= current['cond_adx_strength']
                if PARAMS['trend']['use_adx_direction_filter']:
                    trend_ok_buy &= current['cond_adx_dir_buy']
                    trend_ok_sell &= current['cond_adx_dir_sell']

                # Entry Decision
                entry_score_threshold = PARAMS['core']['entry_score_threshold']
                if trend_ok_buy and scaled_score_bar >= entry_score_threshold:
                    if fib_bounce_buy_cond:
                        entry_signal = "Fib Bounce Long"
                    elif ema_bounce_buy_cond:
                        entry_signal = "EMA Bounce Long"
                    elif bb_bounce_buy_cond:
                        entry_signal = "BB Bounce Long"
                    elif vol_breakout_buy_cond:
                        entry_signal = "Vol Breakout Long"
                    elif df.loc[current_idx, 'cond_ema_signal_buy'] and df.loc[current_idx, 'cond_rsi_buy']:
                        entry_signal = "Basic Long"
                elif trend_ok_sell and scaled_score_bar <= (10.0 - entry_score_threshold):
                    if fib_bounce_sell_cond:
                        entry_signal = "Fib Bounce Short"
                    elif ema_bounce_sell_cond:
                        entry_signal = "EMA Bounce Short"
                    elif bb_bounce_sell_cond:
                        entry_signal = "BB Bounce Short"
                    elif vol_breakout_sell_cond:
                        entry_signal = "Vol Breakout Short"
                    elif df.loc[current_idx, 'cond_ema_signal_sell'] and df.loc[current_idx, 'cond_rsi_sell']:
                        entry_signal = "Basic Short"
                ...
                if entry_signal:
                    ...
                    trade_state.entry_price = entry_price_adj
                    trade_state.entry_index = i
                    trade_state.time_in_trade = 0
                    
                    if "Long" in entry_signal:
                        trade_state.position = 'Long'
                        trade_state.highest_high_in_trade = current['high']
                        initial_stop = current['low'] - current['atr'] * PARAMS['core']['trailing_stop_atr_multiplier']
                        trade_state.trailing_stop = initial_stop
                        df.loc[current_idx, 'signal'] = 'Long'
                        df.loc[current_idx, 'stop_loss'] = initial_stop
                    elif "Short" in entry_signal:
                        trade_state.position = 'Short'
                        trade_state.lowest_low_in_trade = current['low']
                        initial_stop = current['high'] + current['atr'] * PARAMS['core']['trailing_stop_atr_multiplier']
                        trade_state.trailing_stop = initial_stop
                        df.loc[current_idx, 'signal'] = 'Short'
                        df.loc[current_idx, 'stop_loss'] = initial_stop
                    
                    df.loc[current_idx, 'position'] = trade_state.position
                    df.loc[current_idx, 'entry_price'] = trade_state.entry_price
                    df.loc[current_idx, 'entry_signal_type'] = entry_signal
                    df.loc[current_idx, 'trailing_stop'] = trade_state.trailing_stop
...
def plot_signals(df: pd.DataFrame, save_path: Optional[Path] = None):
    ...
    long_entries = df[df['signal'] == 'Long']
    short_entries = df[df['signal'] == 'Short']
    long_exits = df[df['signal'].str.contains('Exit Long', na=False)]
    short_exits = df[df['signal'].str.contains('Exit Short', na=False)]
    ...
    ax1.scatter(long_exits.index, long_exits['exit_price'], marker='x', color='fuchsia', s=80, label='Exit', zorder=5)
    ...
    ax1.set_title('Price, EMAs, Signals, Stops & Targets')
    ...
    ax_score.set_title('Confidence Score')
    ax_score.set_ylabel('Score')
    ax_score.grid(True, which='both', linestyle=':', linewidth=0.5)

    # --- RSI Chart ---
    ax_rsi = axes[2]
    ax_rsi.plot(df['rsi'], label='RSI', color='orange', linewidth=1.0)
    ax_rsi.axhline(50, linestyle='--', color='gray', alpha=0.5)
    ax_rsi.axhline(PARAMS['rsi']['rsi_buy_level'], linestyle=':', color='green', alpha=0.4)
    ax_rsi.axhline(PARAMS['rsi']['rsi_sell_level'], linestyle=':', color='red', alpha=0.4)
    
    bull_div = df[df['bullish_rsi_div']]
    bear_div = df[df['bearish_rsi_div']]
    ...
    ax_rsi.grid(True, which='both', linestyle=':', linewidth=0.5)

    # --- MACD Chart ---
    ax_macd = axes[3]
    ax_macd.plot(df['macd'], label='MACD', color='blue', linewidth=1.0)
    ax_macd.plot(df['macd_signal'], label='Signal', color='red', alpha=0.8, linewidth=1.0)
    
    colors = ['g' if v >= 0 else 'r' for v in df['macd_hist']]
    ax_macd.bar(df.index, df['macd_hist'], label='Histogram', color=colors, alpha=0.5)
    
    ax_macd.axhline(0, linestyle='--', color='gray', alpha=0.5)
    ax_macd.legend(loc='upper left')
    ax_macd.set_title('MACD')
    ax_macd.set_ylabel('MACD')
    ax_macd.grid(True, which='both', linestyle=':', linewidth=0.5)
...
def main():
    ...
    try:
        ...
        df_processed = process_signals(df.copy())
        ...
        # ADDED: Perform analysis and print summary
        analyzer = EnhancedSignalAnalyzer()  # Create analyzer instance
        analyzer.analyze_trades(df_processed)  # Analyze the processed data
        analyzer.print_summary()  # Print the performance summary
        ...
        logger.error(f"Input file '{input_file}' is empty or corrupt.")
