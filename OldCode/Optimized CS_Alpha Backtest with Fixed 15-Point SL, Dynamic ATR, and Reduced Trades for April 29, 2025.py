...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ATR_5'] = ta.atr(df['high'], df['low'], df['close'], length=7, mamode='ema').ffill().fillna(15.0)
...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    df.loc[df['ADX'] > 28, 'Market_Condition'] = 'trending'
    df.loc[df['ATR_5'] > (atr_mean + 0.3 * atr_std), 'Market_Condition'] = 'volatile'
    df.loc[df.index.time <= time(10, 30), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        # Compute scores with SuperTrend and EMA weight
        buy_score = (
            int(r['close'] > p['close']) +
            int(abs(r['close'] - p['close']) > 1) +
            int(r['RSI_10'] > 50) +
            int(r['MACD'] > r['MACD_signal']) +
            2.0 * int(df['SuperTrend_dir'].iloc[i] == 1) +
        ...
        if not morning_done and position is None and i == 0:
            ...
            stop = entry - r['ATR_5'] * 3.0 if side == 'buy' else entry + r['ATR_5'] * 3.0
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    ...
                    if pnl > 0:
                        new_stop = entry + 0.3 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.3 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop
            ...
            rsi_exit = r['RSI_10'] > 80 or r['RSI_10'] < 20
            max_hold = ((bar_count >= 12 and r['Market_Condition'] == 'ranging') or
                        (bar_count >= 18 and r['Market_Condition'] in ['trending', 'volatile']) or
        ...
        if buy_score >= 4.5 and r['ATR_5'] > 15 and df['SuperTrend_dir'].iloc[i] == 1 and momentum:
            entry = r['open']
        ...
        if sell_score >= 5.0 and r['ATR_5'] > 15 and df['SuperTrend_dir'].iloc[i] == -1 and momentum:
            entry = r['open']
            stop = entry + 15.0  # Fixed 15-point SL
            target = entry - 30.0  # Fixed 30-point TP
