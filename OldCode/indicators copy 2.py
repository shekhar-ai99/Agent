...
class IndicatorCalculator:
    ...
    DEFAULT_PARAMS = {
        ...
        'vwap_enabled': True,
        'vwma_period': 20,
        'vwap_type': 'cumulative',  # or 'session'
        'vol_ma_enabled': True
    ...
    def calculate_dmi_adx(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            if adx_results is None or adx_results.empty:
                logger.error("pandas-ta adx calculation returned empty results.")
                df['plus_di'] = df['minus_di'] = df['adx'] = np.nan
                return df

            dmp_col = f'DMP_{dmi_len}'
            dmn_col = f'DMN_{dmi_len}'
            adx_col = f'ADX_{adx_smooth}'
            cols_to_rename = {}
            final_cols = ['plus_di', 'minus_di', 'adx']

            if dmp_col in adx_results.columns:
                cols_to_rename[dmp_col] = final_cols[0]
            else:
                logger.warning(f"Column {dmp_col} not found in ta.adx results.")
            if dmn_col in adx_results.columns:
                cols_to_rename[dmn_col] = final_cols[1]
            else:
                logger.warning(f"Column {dmn_col} not found in ta.adx results.")
            if adx_col in adx_results.columns:
                cols_to_rename[adx_col] = final_cols[2]
            else:
                logger.warning(f"Column {adx_col} not found in ta.adx results.")
            ...
            logger.error(f"AttributeError calculating DMI/ADX. Is 'import pandas_ta as ta' correct and at the top?", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Error calculating DMI/ADX: {str(e)}", exc_info=True)
            raise
    def calculate_vwma(self, df: pd.DataFrame, period: int = 20) -> pd.DataFrame:
        try:
            price_volume = df['close'] * df['volume']
            vwma = price_volume.rolling(window=period).sum() / df['volume'].rolling(window=period).sum().replace(0, np.nan)
    ...
    def calculate_volume_ma(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            df[f'vol_ma_{self.params["vol_ma_len"]}'] = df['volume'].rolling(self.params['vol_ma_len'], min_periods=1).mean()
            return df
        except Exception as e:
    ...
    def calculate_volume_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            df['volume'] = pd.to_numeric(df['volume'], errors='coerce').astype(float).fillna(0)
            df['obv'] = (np.sign(df['close'].diff().fillna(0)) * df['volume']).cumsum()
            df['obv_ema'] = df['obv'].ewm(span=self.params['obv_ema_period'], adjust=False).mean()
         
            if self.params['vwap_enabled']:
                if 'datetime' not in df.columns:
                    raise ValueError("Column 'datetime' is required for daily VWAP calculation.")
                ...
                for date, group in df.groupby('date'):
                    ...
                    vwap = (group['tpv'].cumsum() / vol_cumsum.replace(0, np.nan)).ffill()
                    vwap_values.extend(vwap)

                df['vwap'] = vwap_values
                df.drop(columns=['date', 'tpv'], inplace=True)
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> None:
        ...
        if len(df) < min_required_rows:
            logger.warning(f"Limited data points ({len(df)}) may affect indicator accuracy (min ~{min_required_rows} recommended)")
    ...
    def calculate_bollinger_bands(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            df['bollinger_pctb'] = (df['close'] - df['bollinger_lower']) / (df['bollinger_upper'] - df['bollinger_lower']).replace(0, 1)
            return df
        except Exception as e:
            logger.error(f"Error calculating Bollinger Bands: {str(e)}")
            raise
    def calculate_session_vwap(self, df: pd.DataFrame, session_col='date') -> pd.DataFrame:
        try:
            if session_col not in df.columns:
                df[session_col] = df.index.date if isinstance(df.index, pd.DatetimeIndex) else df.index
            df['vwap'] = df.groupby(session_col).apply(
                lambda g: ((g['close'] * g['volume']).cumsum() / g['volume'].cumsum().replace(0, np.nan))
            ).reset_index(level=0, drop=True)
            return df
        except Exception as e:
            logger.error(f"Error calculating session-based VWAP: {e}")
            raise
