...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ATR_5'] = ta.atr(df['high'], df['low'], df['close'], length=5, mamode='ema').ffill().fillna(15.0).clip(upper=25.0)
    ...
    df['volume_ma_10'] = df['volume'].rolling(window=10, min_periods=1).mean().ffill().fillna(df['volume'])
    return df
...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    df.loc[df['ATR_5'] > (atr_mean + 0.4 * atr_std), 'Market_Condition'] = 'volatile'
    df.loc[df.index.time <= time(10, 45), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        # Compute scores with enhanced sell conditions
        buy_score = (
            int(r['close'] > p['close']) +
            ...
            int(r['EMA7'] > r['EMA14']) +
            int(r['volume'] > r['volume_ma_10'] * 1.2)
        )
        sell_score = (
            int(r['close'] < p['close']) +
            int(abs(r['close'] - p['close']) > 1) +
        ...
        if not morning_done and position is None and i == 0:
            ...
            stop = entry - r['ATR_5'] * 3.5 if side == 'buy' else entry + r['ATR_5'] * 3.5
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    ...
                    if pnl > 0:
                        new_stop = entry + 0.2 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.2 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop
            ...
            flip = (side == 'buy' and sell_score >= 6.0) or (side == 'sell' and buy_score >= 6.0)
        ...
        momentum = abs(r['close'] - p['close']) > r['ATR_5'] * 0.6 if i > 0 else False
        if (buy_score >= 4.5 and r['ATR_5'] > 18 and df['SuperTrend_dir'].iloc[i] == 1 and
            momentum and r['volume'] > r['volume_ma_10'] * 1.2):
            entry = r['open']
        ...
        if (sell_score >= 5.5 and r['ATR_5'] > 18 and df['SuperTrend_dir'].iloc[i] == -1 and
            momentum and r['volume'] > r['volume_ma_10'] * 1.2):
            entry = r['open']
            stop = entry + 15.0  # Fixed 15-point SL
            target = entry - 30.0  # Fixed 30-point TP
