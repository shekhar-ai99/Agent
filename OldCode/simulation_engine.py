...
class SimpleBacktester:
    ...
    def _log_and_prepare_final_results(self, df: pd.DataFrame, original_timeframe: str, cumulative_pnl: float,
                                    ...
                                    incremental_exit_reason_counter: Dict,
        ...
        try:
            ...
            # Updated Performance Score Calculation
            if trade_count == 0:
                performance_score_final = -np.inf
                self.sim_logger.warning("No trades executed. Assigning performance score of -inf.")
            elif trade_count < getattr(config, "MIN_TRADES_FOR_RELIABLE_SCORE", 5):
                performance_score_final = -100.0
                self.sim_logger.warning(f"Low trade count ({trade_count}) for score calculation. Min required: {getattr(config, 'MIN_TRADES_FOR_RELIABLE_SCORE', 5)}. Assigning low score.")
            else:
                # Dynamic normalization using ATR
                atr_normalizer = df['atr'].mean() * 100 if 'atr' in df.columns and not df['atr'].empty else 500.0
                normalized_pnl = cumulative_pnl / atr_normalizer
                normalized_drawdown = max_drawdown / (atr_normalizer * 2) if max_drawdown > 0 else 0.0
                
                # Calculate Sharpe ratio
                returns = [t.get('PnL_Net', 0) / t.get('EntryPrice', 1) for t in trades_details_list if t.get('PnL_Net') and t.get('EntryPrice')]
                sharpe_ratio = np.mean(returns) / np.std(returns) * np.sqrt(252) if returns and np.std(returns) > 0 else 0.0
                normalized_sharpe = min(max(sharpe_ratio, -2.0), 2.0) / 2.0  # Normalize to [-1, 1]
                
                # Trade count factor (linear)
                trade_count_factor = min(trade_count / getattr(config, "TARGET_TRADE_COUNT_FOR_SCORE", 20), 1.0)
                
                # Adjust weights for expiry days and Afternoon session
                drawdown_weight = -0.3 if is_expiry_flag_from_data and session_from_data == "Afternoon" else -0.2
                volatility_factor = 0.8 if vol_stat_from_data == "Volatile" else 1.0
                ...
                ) * trade_count_factor * volatility_factor * 100
                
                performance_score_final = round(float(performance_score_calculated), 4)
                self.sim_logger.info(f"Calculated Performance Score: {performance_score_final} (PnL: {normalized_pnl}, Win Rate: {win_rate/100.0}, Sharpe: {normalized_sharpe}, Drawdown: {normalized_drawdown}, Trade Factor: {trade_count_factor}, Volatility Factor: {volatility_factor})")
            ...
            if optuna_trial_params:
                ...
                #         params_logged_for_run["strategy_base_params"] = "default_or_fixed"
                    ...
                    # This is an "Agent Mode" run (using RuleBasedAgent, not an Optuna trial)
                    params_logged_for_run["run_mode"] = "RuleBasedAgent"
                    params_logged_for_run["agent_parameter_source"] = "Dynamic (MongoDB lookup per context)"
                    
                    # Optional: If your RuleBasedAgent has a method to provide a summary of its static config
                    # if hasattr(self.agent, 'get_configuration_summary'):
                    #     try:
                    #         params_logged_for_run.update(self.agent.get_configuration_summary())
                    #     except Exception as e_agent_summary:
                    #         self.sim_logger.warning(f"Could not get agent configuration summary: {e_agent_summary}")
                    
                    # Log the backtester's base default multipliers which act as ultimate fallbacks
                    # if the agent didn't provide valid ones for a specific bar during the simulation.
                    params_logged_for_run.update({
                        "backtester_default_sl_mult": sl_mult_used, # This is self.default_sl_mult from SimpleBacktester
                        "backtester_default_tp_mult": tp_mult_used, # This is self.default_tp_mult from SimpleBacktester
                        "backtester_default_tsl_mult": tsl_mult_used if tsl_mult_used is not None else "N/A"
                    })
                    # self.strategy_name here would likely be "RuleBasedAgentRun" or similar,
                    # set during SimpleBacktester initialization when an agent is provided.
            elif self.strategy_func:
                    # This is a run with a simple strategy_func, not an Optuna trial, and not an agent
                    params_logged_for_run.update({
                        "sl_mult": sl_mult_used, # self.default_sl_mult
                        "tp_mult": tp_mult_used, # self.default_tp_mult
                        "tsl_mult": tsl_mult_used if tsl_mult_used is not None else "N/A"
                    })
                    params_logged_for_run["strategy_parameter_source"] = "Fixed (from backtester defaults)"
                    # self.strategy_name here would be the name provided when SimpleBacktester was
                    # initialized with the strategy_func.
            else:
                    # Should not happen if __init__ enforces agent or strategy_func
                    self.sim_logger.warning("params_logged_for_run: Neither Optuna, Agent, nor Strategy Func active. Logging basic defaults.")
                    params_logged_for_run.update({
                        "sl_mult": sl_mult_used, "tp_mult": tp_mult_used,
                        "tsl_mult": tsl_mult_used if tsl_mult_used is not None else "N/A",
                        "parameter_source": "Unknown/Fallback"
                    })    

            # Avoid storing -inf in MongoDB
            mongo_safe_score = performance_score_final
            if mongo_safe_score == float('-inf') or np.isneginf(mongo_safe_score):
                mongo_safe_score = -9999.0  # Or any other sentinel value like 0.0 or -100.0
                self.sim_logger.warning(f"Performance score is -inf. Replacing with {mongo_safe_score} for MongoDB storage.")
            ...
            log_backtest_run_results(
                ...
                performance_metrics={
                    ...
                    "trade_count": int(trade_count), "win_rate": float(round(win_rate, 2)),
            ...
            return_dict = {
                "total_pnl": float(cumulative_pnl), "trade_count": int(trade_count),
                "win_rate": float(round(win_rate, 2)), 
                "exit_reasons": final_exit_reasons_summary,
                "performance_score": performance_score_final,
                "max_drawdown": float(max_drawdown), "profit_factor": float(round(profit_factor, 2)),
