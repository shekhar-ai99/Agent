...
# 1) Hardcode data (66 rows for 2025-04-29)
...
# Forward-fill ATR for initial bars to avoid 0.00
df['ATR_10'] = df['ATR_10'].fillna(method='ffill').fillna(30.0)  # Default ATR=30.0 for first bar
for L in [7, 14, 21, 50]:
    df[f'EMA{L}'] = ta.ema(df['close'], length=L).fillna(method='ffill').fillna(df['close'])
...
def backtest_cs_alpha(df):
    ...
    for i in range(len(df)):
        ...
        # --- End-of-day exit at 15:25:00 ---
        if position and t == time(15, 25):
            side = position['side']
            entry = position['entry']
            trade_id = position['id']
            exit_price = row['close']
            profit = (exit_price - entry) if side == 'buy' else (entry - exit_price)
            log.append(f"{t} EXIT    {side.upper()} | EXIT={exit_price:.2f} | P&L={profit:.2f} EOD-CLOSE | Trade {trade_id}")
            trades.append((trade_id, etime, side, entry, row.name, exit_price, profit))
            position = None
