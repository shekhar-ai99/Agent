...
class HistoricalDataFetcher:
    def __init__(self, interval: str,symbol,):
        self.SYMBOL = symbol  # Token for NIFTY 50
    ...
    def fetch_historical_data(self, smart_api: SmartConnect, days: int = 360) -> pd.DataFrame:
        """Fetch historical candle data"""
        try:
            ist = pytz.timezone('Asia/Kolkata')
            now = datetime.now(ist)
            start_time = now - timedelta(days=days)
            print("Getting data from :",start_time," to :",now)
            params = {
                "exchange": "NSE",
                "symboltoken": self.SYMBOL,
...
def main(output: str):
    ...
    try:
        ...
        for interval_key, suffix in intervals.items():
            ...
            fetcher = HistoricalDataFetcher(interval=interval_key,symbol="99926000")
