...
# Try to import pandas_ta, and install it if it's not found
try:
    ...
    try:
        ...
        print(f"Error installing pandas_ta: {e}")
...
df['AlphaTrend'] = 0.0  # Initialize AlphaTrend column
...
df.loc[df['EMA_slope'] > 0, 'Market_Condition'] = 'trending' #corrected EMA Slope
...
def backtest_strategy(df, strategy_name, condition):
    ...
    for i in range(1, len(df)):
        ...
        # Recalculate atr_mean and atr_std in each iteration
        atr_mean, atr_std = df['ATR'][:i].mean(), df['ATR'][:i].std()
        ...
        if strategy_name == 'AlphaTrend_ST_RSI':
            ...
            if position is None and entry_score >= 3.5 and current_time < time(15, 0) and row['close'] > row['Pivot']:
                position = {'signal': 'buy', 'entry_price': row['open'], 'entry_time': row.name, 'strategy': strategy_name}
                highest_price = row['high']
