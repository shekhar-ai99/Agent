...
def backtest_cs_alpha(df):
    ...
    for i in range(len(df)):
        ...
        if not morning_done and i == 0:
            side        = 'buy'     # or detect gap-up/down here
            entry       = row['open']
            first_rng   = row['high'] - row['low']
            sl_price    = entry - 0.5 * first_rng
            entry_time0 = row.name
            print(f"ENTER {t} | {side.upper():<4} ENTRY={entry:.2f} | SL={sl_price:.2f} (½×1st-bar range)")
            position     = {'side': side, 'entry': entry}
            morning_done = True
            continue

        # --- Trailing‐SL exit for the morning trade ---
        if position and entry_time0 is not None:
            side  = position['side']
            entry = position['entry']

            # update trailing SL
            if side == 'buy':
                # for long, move SL up to today's bar low if higher
                sl_price = max(sl_price, row['low'])
                # if price falls to/below SL → exit
                if row['low'] <= sl_price:
                    profit = sl_price - entry
                    print(f"EXIT  {t} | PROFIT={profit:.2f} | EXIT={sl_price:.2f} | TSL (morning hit)")
                    trades.append((entry_time0, side, entry, row.name, sl_price, profit))
                    position     = None
                    entry_time0  = None
                    sl_price     = None
                    continue
            else:
                # for short, trail SL down to bar high if lower
                sl_price = min(sl_price, row['high'])
                if row['high'] >= sl_price:
                    profit = entry - sl_price
                    print(f"EXIT  {t} | PROFIT={profit:.2f} | EXIT={sl_price:.2f} | TSL (morning hit)")
                    trades.append((entry_time0, side, entry, row.name, sl_price, profit))
                    position     = None
                    entry_time0  = None
            ...
            # optional: a final cut-off time in case SL never hits (e.g. 09:50)
            if t == time(9,50):
                exit_price = row['close']
                profit     = (exit_price - entry) if side=='buy' else (entry - exit_price)
                print(f"EXIT  {t} | PROFIT={profit:.2f} | EXIT={exit_price:.2f} | Slot cut-off")
                trades.append((entry_time0, side, entry, row.name, exit_price, profit))
                position     = None
                entry_time0  = None
                sl_price     = None
                continue

        # --- Mid-day scoring entries (unchanged) ---
        if position is None and i >= 2:
            prev2 = df.iloc[i-2]
            cb    = int(row['close'] > prev2['close'])
            cs    = int(row['close'] < prev2['close'])
            ad    = int(abs(row['close']-prev2['close']) > 1)
            rb    = int(row['RSI_10'] > 50)
            rs    = int(row['RSI_10'] < 50)
            mb    = int(row['MACD'] > row['MACD_signal'])
            ms    = int(row['MACD'] < row['MACD_signal'])
            sb    = int(row['SuperTrend_dir'] == 1)
            ss    = int(row['SuperTrend_dir'] == -1)
            pb    = 0.5 if row['RSI_slope'] > 0 else 0
            ps    = 0.5 if row['RSI_slope'] < 0 else 0

            buy_score  = cb + ad + rb + mb + sb + pb
            sell_score = cs + ad + rs + ms + ss + ps

            if buy_score >= 3.5:
                entry = row['open']
                print(f"ENTER {t} | BUY  ENTRY={entry:.2f} | SCORE={buy_score:.1f}")
                position = {'side':'buy','entry':entry}
                continue
            if sell_score >= 3.5:
                entry = row['open']
                print(f"ENTER {t} | SELL ENTRY={entry:.2f} | SCORE={sell_score:.1f}")
                position = {'side':'sell','entry':entry}
                continue

        # --- Fixed-slot exits for all other trades (unchanged) ---
        if position and entry_time0 is None and t in [
            ...
            trades.append((row.name, side, entry, row.name, exit_price, profit))
            position = None
