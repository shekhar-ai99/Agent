...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ATR_5'] = ta.atr(df['high'], df['low'], df['close'], length=7, mamode='ema').ffill().fillna(15.0).clip(upper=22.0)
    df['ATR_5'] = df['ATR_5'].ewm(span=3).mean()  # Smooth ATR
    df['ADX'] = ta.adx(df['high'], df['low'], df['close'], length=14)['ADX_14'].ffill().fillna(0)
    for L in [7, 14, 21, 50]:
        df[f'EMA{L}'] = ta.ema(df['close'], length=L).ffill().fillna(df['close'])
...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    df.loc[df['ADX'] > 22, 'Market_Condition'] = 'trending'
    df.loc[df['ATR_5'] > (atr_mean + 0.3 * atr_std), 'Market_Condition'] = 'volatile'
    df.loc[df.index.time <= time(11, 0), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        buy_score = (
            ...
            2.5 * int(df['SuperTrend_dir'].iloc[i] == 1) +
        ...
        sell_score = (
            ...
            (0.5 if r['RSI_slope'] < 0 else 0) +
            int(r['EMA7'] < r['EMA14']) +
            int(r['close'] < p['close'] - r['ATR_5'] * 0.3)
        )
        ...
        if not morning_done and position is None and i == 0:
            ...
            stop = entry - r['ATR_5'] * 4.0 if side == 'buy' else entry + r['ATR_5'] * 4.0
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    # Morning TSL: (prev_close - prev_open) * 0.2 + prev_open
                    prev_open = p['open']
                    prev_close = p['close']
                    new_stop = (prev_close - prev_open) * 0.2 + prev_open
                else:
                    # ATR-based TSL for mid-day trades
                    if pnl > 0:
                        new_stop = entry + 0.15 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.15 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop
            ...
            # Define TP (30 points for mid-day trades, none for morning)
            ...
            # 2b) Target hit (mid-day trades only)
            ...
            # 2c) Score-flip, RSI, or max hold exit
            flip = (side == 'buy' and sell_score >= 5.5) or (side == 'sell' and buy_score >= 5.5)
            rsi_exit = (r['RSI_10'] > 85 or r['RSI_10'] < 15) and bar_count >= 3
            max_hold = ((bar_count >= 10 and r['Market_Condition'] == 'ranging') or
                        (bar_count >= 15 and r['Market_Condition'] in ['trending', 'volatile']) or
                        (t >= time(9, 45) and position['time'] == df.index[0]))
            if flip or rsi_exit or max_hold:
                if flip and pnl > 0:
                    exit_price = entry + 0.5 * pnl if side == 'buy' else entry - 0.5 * pnl
                else:
        ...
        momentum = abs(r['close'] - p['close']) > r['ATR_5'] * 0.4 if i > 0 else False
        if (buy_score >= 4.5 and r['ATR_5'] > 14 and df['SuperTrend_dir'].iloc[i] == 1 and momentum):
            entry = r['open']
        ...
        if (sell_score >= 5.5 and r['ATR_5'] > 14 and df['SuperTrend_dir'].iloc[i] == -1 and momentum):
            entry = r['open']
            stop = entry + 15.0  # Fixed 15-point SL
            target = entry - 30.0  # Fixed 30-point TP
