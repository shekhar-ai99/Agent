...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ATR_5'] = ta.atr(df['high'], df['low'], df['close'], length=8, mamode='ema').ffill().fillna(15.0).clip(upper=20.0)
    df['ATR_5'] = df['ATR_5'].ewm(span=5).mean()  # Smooth ATR
...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    df.loc[df['ATR_5'] > (atr_mean + 0.25 * atr_std), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        sell_score = (
            ...
            (0.5 if r['RSI_slope'] < 0 else 0) +
            int(r['EMA7'] < r['EMA14']) +
            int(r['close'] < p['close'] - r['ATR_5'] * 0.4)
        )
        ...
        if not morning_done and position is None and i == 0:
            ...
            stop = entry - r['ATR_5'] * 4.5 if side == 'buy' else entry + r['ATR_5'] * 4.5
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    # Morning TSL: (prev_close - prev_open) * 0.15 + prev_open
                    prev_open = p['open']
                    prev_close = p['close']
                    new_stop = (prev_close - prev_open) * 0.15 + prev_open
                else:
                    # ATR-based TSL for mid-day trades
                    if pnl > 0:
                        new_stop = entry + 0.1 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.1 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop
            ...
            flip = (side == 'buy' and sell_score >= 5.0) or (side == 'sell' and buy_score >= 5.0)
            rsi_exit = (r['RSI_10'] > 80 or r['RSI_10'] < 20) and bar_count >= 3
            max_hold = ((bar_count >= 10 and r['Market_Condition'] == 'ranging') or
                        (bar_count >= 12 and r['Market_Condition'] in ['trending', 'volatile']) or
                        (t >= time(9, 30) and position['time'] == df.index[0]))
            if flip or rsi_exit or max_hold:
                if flip and pnl > 0:
                    exit_price = entry + 0.6 * pnl if side == 'buy' else entry - 0.6 * pnl
                else:
        ...
        momentum = abs(r['close'] - p['close']) > r['ATR_5'] * 0.5 if i > 0 else False
        if (buy_score >= 4.5 and r['ATR_5'] > 16 and df['SuperTrend_dir'].iloc[i] == 1 and momentum and r['close'] > r['EMA_21']):
            entry = r['open']
        ...
        if (sell_score >= 6.0 and r['ATR_5'] > 16 and df['SuperTrend_dir'].iloc[i] == -1 and momentum and r['close'] < r['EMA_21']):
            entry = r['open']
            stop = entry + 15.0  # Fixed 15-point SL
            target = entry - 30.0  # Fixed 30-point TP
