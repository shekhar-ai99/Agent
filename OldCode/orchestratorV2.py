...
# Import the backtesting workflow from main_workflow.py
...
class PipelineOrchestrator:
    ...
    def run_historical_phase(self) -> bool:
        """Copy pre-downloaded historical data from dataset folder to DATA_DIR."""
        dataset_dir = self.DATASET_DIR
        expected_timeframes = ["1min", "3min", "5min", "15min"]
        success = True

        if not dataset_dir.exists():
            self.logger.error(f"Dataset directory {dataset_dir} does not exist.")
            return False

        self.logger.info(f"Copying historical data from {dataset_dir} to {self.DATA_DIR}")
        for timeframe in expected_timeframes:
            source_file = dataset_dir / f"nifty_historical_data_{timeframe}.csv"
            dest_file = self.DATA_DIR / f"nifty_historical_data_{timeframe}.csv"
            
            if not source_file.exists():
                self.logger.error(f"Historical data file {source_file} not found.")
                success = False
                continue
        ...
        if not success:
            self.logger.error("Historical data copying phase failed for one or more files.")
        return success

    def run_indicator_phase(self) -> bool:
        """Run the indicator calculator for each historical data file."""
        success = True
    ...
    def run_backtesting_phase(self) -> bool:
        ...
        try:
            ...
            run_full_backtest_workflow.run_full_backtest_workflow(
