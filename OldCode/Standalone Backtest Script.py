...
strategies = {
    'Momentum': lambda row, prev_row: row['ADX'] > 25 and row['EMA7'] > row['EMA14'] and row['close'] > row['AlphaTrend'] and row.name.time() < time(14, 0),
    ...
    'Scalping': lambda row, prev_row: row['ADX'] < 20 and abs(row['EMA7'] - row['EMA14']) < 0.3 * row['ATR'] and row['ATR'] > 1.5 * atr_mean,
    ...
    'Bull_Flag': lambda row, prev_row: row['ADX'] > 20 and row['ATR'] > 1.5 * atr_mean,
    ...
    for i in range(1, len(df)):
        ...
        if strategy_name == 'AlphaTrend_ST_RSI':
            ...
            buy_conditions = [
                ...
                (50 < row['RSI'] < 70) or (row['RSI'] > 40),
                row['SuperTrend_dir'] == 1 or (row['SuperTrend_dir'] == 0 and prev_row['SuperTrend_dir'] < 1),
                row['RSI_slope'] > 0,
            ...
            if position is None and buy_score >= 5:
                ...
                if current_time < time(9, 30):
                    stop_loss = highest_price - (0.5 * (prev_row['high'] - prev_row['low']) + 2.5 * row['ATR'])
                else:
                    stop_loss = highest_price - (2.0 * row['ATR'] if row['RSI'] > 75 else 3.5 * row['ATR'])
                if row['low'] <= stop_loss:
                    profit = stop_loss - position['entry_price']
                    trades.append({
                        'strategy': strategy_name,
                        'market_condition': row['Market_Condition'],
                        'profit': profit,
        ...
        if position and strategy_name != 'AlphaTrend_ST_RSI':
            highest_price = max(highest_price, row['high'])
            if strategy_name == 'Gap_and_Go' and current_time >= time(10, 0):
                profit = row['close'] - position['entry_price']
                trades.append({
                    'strategy': strategy_name,
                    'market_condition': row['Market_Condition'],
                ...
                stop_loss = highest_price - atr_mult * row['ATR'] if strategy_name != 'Breakout' else highest_price - 3.0 * row['ATR']
