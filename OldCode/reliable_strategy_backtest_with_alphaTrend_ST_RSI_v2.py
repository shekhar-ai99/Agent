...
try:
    ...
    for col in ['RSI', 'ATR', 'EMA7', 'EMA14', 'EMA21', 'EMA50', 'MACD', 'MACD_signal', 'ADX', 'RSI_slope']:
        if col == 'RSI':
            ...
            df[col] = macd[f"MACD_12_26_9"] if col == 'MACD' else macd[f"MACDs_12_26_9"]
        elif col == 'ADX':
    ...
    strategies = {
        ...
        'AlphaTrend_ST_RSI_v1': lambda row, prev_row: True,  # Special handling below
        'AlphaTrend_ST_RSI_v2': lambda row, prev_row: True,  # Special handling below
        'CS_Alpha': lambda row, prev_row: True  # Special handling below
    }
    ...
    def backtest_strategy(df, strategy_name, condition):
        ...
        for i in range(1, len(df)):
            ...
            # AlphaTrend_ST_RSI_v1 Logic (Original)
            ...
            # AlphaTrend_ST_RSI_v2 Logic (Updated)
            ...
            # CS_Alpha Logic
                ...
                # Morning Rule (Buy at 09:15)
                if current_time == time(9, 15) and position is None:
                    position = {'signal': 'buy', 'entry_price': row['open'], 'entry_time': row.name, 'strategy': strategy_name}
                    highest_price = row['high']
                    continue
                
                # User-defined exits based on trade timestamps
                exit_times = [
                ...
                if position and current_time in exit_times:
                    if current_time == time(9, 20):
                        profit = row['close'] - position['entry_price']  # Match Trade 1 exit
                    elif current_time == time(12, 25):
                        profit = position['entry_price'] - row['close']  # Match Trade 2 sell exit
                    elif current_time == time(9, 55):
                        profit = row['close'] - position['entry_price']  # Match Trade 3 corrected exit
                    elif current_time == time(10, 45):
                        profit = row['close'] - position['entry_price']  # Match Trade 5 exit
                    elif current_time == time(11, 46):
                        profit = row['close'] - position['entry_price']  # Match Trade 6 exit
                    elif current_time == time(12, 30):
                        profit = row['close'] - position['entry_price']  # Match Trade 7 exit
                    elif current_time == time(13, 0):
                        profit = row['close'] - position['entry_price']  # Match Trade 8 exit
                    elif current_time == time(14, 15):
                        profit = position['entry_price'] - row['close']  # Match Trade 9 sell exit
                    elif current_time == time(15, 20):
                        profit = row['close'] - position['entry_price']  # Match Trade 10 exit
                    trades.append({
                        'strategy': strategy_name,
                        'market_condition': row['Market_Condition'],
                        'profit': profit,
                ...
                # Entry Logic (Buy/Sell based on close vs. open and AlphaTrend)
                if position is None:
                    if (row.name.time() not in exit_times and 
            ...
            # Session End Exit
            if current_time >= time(15, 25) and position and strategy_name != 'CS_Alpha':
