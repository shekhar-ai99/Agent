...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['ATR_5'] = ta.atr(df['high'], df['low'], df['close'], length=5, mamode='ema').ffill().fillna(15.0)
    df['ATR_5'] = df['ATR_5'].clip(upper=20.0)  # Cap ATR_5 at 20.0
...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    df.loc[df['ATR_5'] > (atr_mean + 0.2 * atr_std), 'Market_Condition'] = 'volatile'
    df.loc[df.index.time <= time(10, 15), 'Market_Condition'] = 'volatile'
    ...
    for i in range(len(df)):
        ...
        # Compute scores with SuperTrend weight
        buy_score = (
            int(r['close'] > p['close']) +
            int(abs(r['close'] - p['close']) > 1) +
            int(r['RSI_10'] > 50) +
            int(r['MACD'] > r['MACD_signal']) +
            1.5 * int(df['SuperTrend_dir'].iloc[i] == 1) +
        ...
        if not morning_done and position is None and i == 0:
            ...
            stop = entry - r['ATR_5'] * 2.5 if side == 'buy' else entry + r['ATR_5'] * 2.5
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                ...
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    ...
                    if pnl > 0:
                        new_stop = entry + 0.5 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.5 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop
        ...
        momentum = abs(r['close'] - p['close']) > r['ATR_5'] * 0.3 if i > 0 else False
        if buy_score >= 4.0 and r['ATR_5'] > 12 and df['SuperTrend_dir'].iloc[i] == 1 and momentum:
            entry = r['open']
        ...
        if sell_score >= 4.5 and r['ATR_5'] > 12 and df['SuperTrend_dir'].iloc[i] == -1 and momentum:
            entry = r['open']
            stop = entry + 15.0  # Fixed 15-point SL
            target = entry - 30.0  # Fixed 30-point TP
