...
def backtest_cs_alpha(df: pd.DataFrame, prev_day_close: float) -> pd.DataFrame:
    ...
    morning_done  = False
    atr_mean      = df['ATR_10'].rolling(window=20, min_periods=1).mean()
    atr_std       = df['ATR_10'].rolling(window=20, min_periods=1).std()
    ...
    for i in range(len(df)):
        ...
        if not morning_done and position is None and i == 0 and r['ATR_10'] > 15:
            entry = r['open']
        ...
        if position:
            ...
            if i > 0:  # Ensure previous bar exists
                atr_scale = r['ATR_10'] / atr_mean.iloc[i] if atr_mean.iloc[i] > 0 else 1.0
                tsl_mult = 0.5 if r['Market_Condition'] == 'volatile' else 0.75 if r['Market_Condition'] == 'trending' else 1.0
                if t <= time(9, 45) and position['time'] == df.index[0]:
                    # Morning TSL: (prev_close - prev_open) * 0.4 + prev_open
                    prev_open  = p['open']
                    prev_close = p['close']
                    new_stop   = (prev_close - prev_open) * 0.4 + prev_open
                else:
                    # ATR-based TSL for mid-day trades
                    if pnl > 0:
                        new_stop = entry + 0.75 * pnl * atr_scale * tsl_mult if side == 'buy' else entry - 0.75 * pnl * atr_scale * tsl_mult
                    else:
                        new_stop = stop

                # Adjust stop based on market condition
                sl_mult = 1.5 if r['Market_Condition'] == 'volatile' else 2.0 if r['Market_Condition'] == 'trending' else 2.5
            ...
            # 2b) Score-flip or RSI exit
            flip = (side == 'buy' and sell_score >= 3.5) or (side == 'sell' and buy_score >= 3.5)
            rsi_exit = r['RSI_10'] > 80 or r['RSI_10'] < 20
            max_hold = bar_count >= 10 and r['Market_Condition'] == 'ranging'
        ...
        if buy_score >= 3.5 and r['ATR_10'] > 15 and df['SuperTrend_dir'].iloc[i] == 1:
            entry = r['open']
            sl_mult = 1.5 if r['Market_Condition'] == 'volatile' else 2.0 if r['Market_Condition'] == 'trending' else 2.5
            stop  = entry - r['ATR_10'] * sl_mult * 1.5
        ...
        if sell_score >= 4.0 and r['ATR_10'] > 15 and df['SuperTrend_dir'].iloc[i] == -1:
            entry = r['open']
            sl_mult = 1.5 if r['Market_Condition'] == 'volatile' else 2.0 if r['Market_Condition'] == 'trending' else 2.5
            stop  = entry + r['ATR_10'] * sl_mult * 1.5
