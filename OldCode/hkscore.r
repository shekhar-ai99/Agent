// @version=6
indicator( title="CS_HK Score Optimized (v5 Enhanced - Corrected v11)", shorttitle="CS Score v5+", overlay=true, max_lines_count=50, max_labels_count=50 )
...
// Function to delete Fibonacci objects
...
f_compute_potential_signals(ema_cross_buy, fib_buy, ema_bounce_buy, bb_bounce_buy, vol_buy, ema_cross_sell, fib_sell, ema_bounce_sell, bb_bounce_sell, vol_sell) =>
	...
	// **MODIFICATION**: Return const strings for alert message compatibility
	string buy_reason = fib_buy ? "Fib" : ema_cross_buy ? "EMA Cross" : vol_buy ? "Vol Breakout" : bb_bounce_buy ? "BB Bounce" : ema_bounce_buy ? "EMA Bounce" : "Other"
	string sell_reason = fib_sell ? "Fib" : ema_cross_sell ? "EMA Cross" : vol_sell ? "Vol Breakout" : bb_bounce_sell ? "BB Bounce" : ema_bounce_sell ? "EMA Bounce" : "Other"
...
var bool is_first_trade_ever = true // **ADDED** Flag for first trade label logic

// --- RR Management State ---
var float rr_target_price = na
var float rr_breakeven_trigger_price = na
var bool sl_moved_to_be = false
// var int last_cleaned_bar = na  // Removed experimental cleanup logic and this variable
...
// **MODIFICATION**: Calculate RSI rising/falling unconditionally
is_rsi_rising = ta.rising(price_rsi, 1)
is_rsi_falling = ta.falling(price_rsi, 1)
...
// **MODIFICATION**: Use pre-calculated is_rsi_rising/falling
...
bool touched_bb_mid_sell = false
...
if use_adx_filter
	...
	if cond_adx_strength_ok // Add strength score if ADX is high, regardless of direction here
...
bool was_in_long_prev = in_long[1] == true  // Equivalent to nz() for bool
bool was_in_short_prev = in_short[1] == true
...
if was_in_long_prev
	...
	// 4. Signal-Based Exits
		...
		exited_long_this_bar := true
		exit_reason_bar := "Take Profit" // ** Changed label per user request **
		exit_price := close
...
if was_in_short_prev
	...
	// 3. Take Profit (RR or Fib)
		...
		exited_short_this_bar := true
		exit_reason_bar := "Short TP" // Use specific TP label
		exit_price := rr_target_price[1]
...
if exited_long_this_bar or exited_short_this_bar
	...
	// Update hypothetical equity (Note: Doesn't account for commission/slippage)
...
if enter_long_condition
	...
	is_first_trade_ever := false // **ADDED** Set flag after first trade
...
if show_auto_fib and valid_fib_range
	...
	f_delete_fib_objects(fib_line_0, fib_line_236, fib_line_382, fib_line_500, fib_line_618, fib_line_786, fib_line_100,
						 fib_label_0, fib_label_236, fib_label_382, fib_label_500, fib_label_618, fib_label_786, fib_label_100) // **MODIFIED CALL**
...
// --- Entry/Exit Shapes and Labels --- **MODIFIED**
// Plot different shapes based on exit reason
plotshape(exited_long_this_bar and not str.contains(exit_reason, "TP") and exit_reason != "Take Profit", "Exit Long Shape (Other)", location=location.abovebar, color=color.new(color.maroon, 50), style=shape.xcross, size=size.small) // X for non-TP exits
plotshape(exited_short_this_bar and not str.contains(exit_reason, "TP") and exit_reason != "Take Profit", "Exit Short Shape (Other)", location=location.belowbar, color=color.new(color.teal, 50), style=shape.xcross, size=size.small) // X for non-TP exits

plotshape(exited_long_this_bar and (str.contains(exit_reason, "Long TP") or exit_reason == "Take Profit"), "Exit Long Shape (TP)", location=location.abovebar, color=color.new(color.lime, 30), style=shape.diamond, size=size.small) // Diamond for TP exits (incl EMA Cross TP)
plotshape(exited_short_this_bar and (str.contains(exit_reason, "Short TP") or exit_reason == "Take Profit"), "Exit Short Shape (TP)", location=location.belowbar, color=color.new(color.red, 30), style=shape.diamond, size=size.small) // Diamond for TP exits (incl EMA Cross TP)
...
// --- Entry Labels --- **MODIFIED** (Uses is_first_trade_ever flag)
if enter_long_condition
	label_text = is_first_trade_ever ? "Buy" : "Re-Buy" // Check the global flag
	label_color = is_first_trade_ever ? color.new(color.green, 20) : color.new(color.lime, 20)
...
if enter_short_condition
	label_text = is_first_trade_ever ? "Sell" : "Re-Sell" // Check the global flag
	label_color = is_first_trade_ever ? color.new(color.red, 20) : color.new(color.maroon, 20)
	text_color = color.white
...
// --- Exit Reason Labels --- **MODIFIED**
// Only plot exit labels if the reason contains "TP" OR is the EMA Cross "Take Profit"
if exited_long_this_bar and not na(exit_reason) and (str.contains(exit_reason, "TP") or exit_reason == "Take Profit")
...
if exited_short_this_bar and not na(exit_reason) and (str.contains(exit_reason, "TP") or exit_reason == "Take Profit")
...
// Exit Alerts (Use the stored 'exit_reason' which now includes "Long TP" / "Short TP" / "Take Profit")
...
alertcondition(exited_long_this_bar and (str.contains(exit_reason, "Long TP") or exit_reason == "Take Profit"), title="Long TP Hit", message="LONG TP HIT: {{ticker}} at {{str.tostring(nz(exit_price, close))}}. PnL: {{str.tostring(last_trade_pnl, '#.##')}}%")
alertcondition(exited_short_this_bar and (str.contains(exit_reason, "Short TP") or exit_reason == "Take Profit"), title="Short TP Hit", message="SHORT TP HIT: {{ticker}} at {{str.tostring(nz(exit_price, close))}}. PnL: {{str.tostring(last_trade_pnl, '#.##')}}%")
