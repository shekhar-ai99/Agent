...
#    # Inside IndicatorCalculator class in indicators.py
...
#                  # Attempt to convert if it's not (e.g., reset from a column)
#                  if 'datetime' in df.columns:
#                       df = df.set_index(pd.to_datetime(df['datetime']))
#                  else:
#                       # Or try converting the existing index
#                       try:
#                            df.index = pd.to_datetime(df.index)
#                            if not isinstance(df.index, pd.DatetimeIndex):
#                                 raise TypeError("Index could not be converted to DatetimeIndex.")
#                       except Exception as e:
#                            raise TypeError(f"DataFrame index is not DatetimeIndex and conversion failed: {e}")

#             # Group by date directly from the index
...
#             return df
#         except Exception as e:
#             logger.error(f"Session VWAP calculation error: {e}", exc_info=True) # Log traceback
...
# indicators.py (Corrected to handle DatetimeIndex for VWAP)
...
import logging
from pathlib import Path
from typing import Optional, Dict, List, Tuple, Union # Make sure Union/Optional are imported
from concurrent.futures import ThreadPoolExecutor, as_completed
from multiprocessing import cpu_count
...
class IndicatorCalculator:
    ...
    DEFAULT_PARAMS = {
        ...
        'vwap_enabled': True,
        'vwma_period': 20,
        'vwap_type': 'cumulative', # 'cumulative' or 'session'
    ...
    def __init__(self, params: Optional[Dict] = None):
        self.params = {**self.DEFAULT_PARAMS, **(params or {})}
        self._validate_params()
        logger.info(f"Initialized IndicatorCalculator with {self.params['parallel_workers']} parallel workers")
    ...
    def _validate_params(self) -> None:
        ...
        if any(p <= 0 for p in all_periods): raise ValueError("All periods must be positive")
        if self.params['bollinger_std'] <= 0: raise ValueError("Bollinger std dev must be positive")
        if self.params['vwap_type'] not in ['cumulative', 'session']: raise ValueError("vwap_type must be 'cumulative' or 'session'")
    ...
    def validate_dataframe(self, df: pd.DataFrame) -> None:
        ...
        # Validate numeric types - allow NaN temporarily, check later if needed
        for col in required_columns: df[col] = pd.to_numeric(df[col], errors='coerce')
        if df[required_columns].isna().any().any(): logger.warning("NaNs found in OHLCV data after coerce.")
        min_period = max(1, max([ # Ensure min_period is at least 1
            *self.params['sma_periods'], *self.params['ema_periods'],
            self.params['rsi_period'], self.params['stochastic_period'],
            self.params['bollinger_period'], self.params['atr_period'],
            self.params['dmi_length'], self.params['adx_smoothing'],
            self.params['vol_ma_len'],
            self.params['macd_params'][1] # Use slow period for MACD requirement
        ]))
        if len(df) < min_period: logger.warning(f"Data length ({len(df)}) less than max period ({min_period}). Indicators may be inaccurate.")
        if (df['high'] < df['low']).any(): raise ValueError("High price less than low price found.")
        if ((df['close'] > df['high']) | (df['close'] < df['low'])).any(): raise ValueError("Close price outside high/low range found.")
        if (df['volume'] < 0).any(): raise ValueError("Negative volume found.")
        logger.debug("DataFrame validation passed.")


    def _parallel_calculate(self, func, df: pd.DataFrame, *args) -> pd.DataFrame:
        # (Keep this function as provided by user)
        with ThreadPoolExecutor(max_workers=self.params['parallel_workers']) as executor:
            future = executor.submit(func, df.copy(), *args)
            return future.result()

    # --- Indicator Calculation Methods (Keep calculate_dmi_adx, _calculate_dmi_adx_helper, calculate_vwma, calculate_volume_ma as provided) ---
    def calculate_dmi_adx(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            dmi_len = self.params['dmi_length']; adx_smooth = self.params['adx_smoothing']
            if not all(col in df for col in ['high', 'low', 'close']): raise ValueError("Missing HLC for DMI/ADX.")
            logger.info(f"Calculating DMI(len={dmi_len})/ADX(smooth={adx_smooth})...")
            if len(df) > 10000 and self.params['enable_parallel']:
                logger.info("Using parallel for DMI/ADX"); return self._parallel_calculate(self._calculate_dmi_adx_helper, df)
            else: return self._calculate_dmi_adx_helper(df)
        except Exception as e: logger.error(f"Error DMI/ADX: {e}", exc_info=True); raise
    def _calculate_dmi_adx_helper(self, df: pd.DataFrame) -> pd.DataFrame:
        dmi_len=self.params['dmi_length']; adx_smooth=self.params['adx_smoothing']
        adx_results=ta.adx(high=df['high'], low=df['low'], close=df['close'], length=dmi_len, adx_length=adx_smooth)
        if adx_results is None or adx_results.empty: logger.error("ADX empty results."); df['plus_di']=df['minus_di']=df['adx']=np.nan; return df
        col_mapping={f'DMP_{dmi_len}':'plus_di', f'DMN_{dmi_len}':'minus_di', f'ADX_{adx_smooth}':'adx'}
        for src, dest in col_mapping.items():
            if src in adx_results.columns: df[dest]=adx_results[src]
            else: logger.warning(f"Col {src} not in ADX results"); df[dest]=np.nan
        return df
    def calculate_vwma(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period=self.params['vwma_period']; price_volume=df['close']*df['volume']; vol_sum=df['volume'].rolling(window=period, min_periods=1).sum()
            df[f'vwma_{period}']=price_volume.rolling(window=period, min_periods=1).sum()/vol_sum.replace(0, np.nan)
            return df
        except Exception as e: logger.error(f"Error VWMA: {e}"); raise
    def calculate_volume_ma(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period=self.params['vol_ma_len']; col_name=f'vol_ma_{period}'; df[col_name]=df['volume'].rolling(period, min_periods=1).mean()
            df['vol_ma']=df[col_name]; return df
        except Exception as e: logger.error(f"Error Volume MA: {e}"); raise

    # --- CORRECTED Volume Indicators ---
    def calculate_volume_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate all volume-related indicators using DatetimeIndex"""
        try:
            df_processed = df.copy() # Work on a copy

            # Ensure required columns for volume indicators are present
            ...
            # Convert volume to numeric efficiently first
            df_processed['volume'] = pd.to_numeric(df_processed['volume'], errors='coerce').fillna(0)

            # --- OBV calculation ---
            price_diff = df_processed['close'].diff().fillna(0)
            df_processed['obv'] = (np.sign(price_diff) * df_processed['volume']).cumsum()
            ...
            # --- VWAP calculation ---
            if self.params['vwap_enabled']:
                # Ensure the index is DatetimeIndex before proceeding
                if not isinstance(df_processed.index, pd.DatetimeIndex):
                     # Attempt conversion if possible (e.g., from object)
                     try:
                ...
                # Now calculate based on type, using the index
                if self.params['vwap_type'] == 'session':
                    df_processed = self.calculate_session_vwap(df_processed) # Pass df with index
                else:
                    # Cumulative VWAP (doesn't need datetime directly)
                    tpv = df_processed['close'] * df_processed['volume']
                    df_processed['vwap'] = tpv.cumsum() / df_processed['volume'].cumsum().replace(0, np.nan)
    ...
    def calculate_session_vwap(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate session-based VWAP using the DatetimeIndex"""
        try:
            # Assumes df.index is DatetimeIndex (checked in calling function)
            df['vwap'] = df.groupby(df.index.date, group_keys=False).apply(
                 lambda group: (group['close'] * group['volume']).cumsum() / group['volume'].cumsum().replace(0, np.nan)
            ).reset_index(level=0, drop=True) # Drop the date level added by groupby
    ...
    # --- Keep other calculation methods as provided ---
    def calculate_trend_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            if self.params['enable_parallel'] and len(df) > 5000:
                with ThreadPoolExecutor(max_workers=self.params['parallel_workers']) as executor:
                    futures = []
                    for period in self.params['sma_periods']: futures.append(executor.submit(lambda p, d: d.assign(**{f'sma_{p}': d['close'].rolling(window=p, min_periods=1).mean()}), period, df.copy()))
                    for period in self.params['ema_periods']: futures.append(executor.submit(lambda p, d: d.assign(**{f'ema_{p}': d['close'].ewm(span=p, adjust=False, min_periods=1).mean()}), period, df.copy()))
                    for future in as_completed(futures): result = future.result(); df[list(result.columns.difference(df.columns))] = result[list(result.columns.difference(df.columns))]
            else:
            ...
            return df
        except Exception as e: logger.error(f"Error trend indicators: {e}"); raise
    def calculate_macd(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            fast, slow, signal = self.params['macd_params']
            if len(df) > 5000 and self.params['enable_parallel']:
                with ThreadPoolExecutor(max_workers=2) as executor:
                    ema_fast_future = executor.submit(lambda: df['close'].ewm(span=fast, adjust=False, min_periods=1).mean())
                    ema_slow_future = executor.submit(lambda: df['close'].ewm(span=slow, adjust=False, min_periods=1).mean())
                    ema_fast = ema_fast_future.result(); ema_slow = ema_slow_future.result()
            else: ema_fast = df['close'].ewm(span=fast, adjust=False, min_periods=1).mean(); ema_slow = df['close'].ewm(span=slow, adjust=False, min_periods=1).mean()
            df['macd'] = ema_fast - ema_slow; df['macd_signal'] = df['macd'].ewm(span=signal, adjust=False, min_periods=1).mean(); df['macd_hist'] = df['macd'] - df['macd_signal']
            return df
        except Exception as e: logger.error(f"Error MACD: {e}"); raise
    def calculate_rsi(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period = self.params['rsi_period']; delta = df['close'].diff()
            if len(delta) < period: logger.warning(f"Insufficient data for RSI ({len(delta)})"); df['rsi']=np.nan; return df
            gain = delta.where(delta > 0, 0); loss = -delta.where(delta < 0, 0)
            avg_gain = gain.ewm(alpha=1/period, adjust=False).mean(); avg_loss = loss.ewm(alpha=1/period, adjust=False).mean()
            rs = avg_gain / avg_loss.replace(0, 1); df['rsi'] = 100 - (100 / (1 + rs)); df['rsi'] = df['rsi'].fillna(50).clip(0, 100)
            return df
        except Exception as e: logger.error(f"Error RSI: {e}"); raise
    def calculate_stochastic(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period = self.params['stochastic_period']
            low_min = df['low'].rolling(window=period, min_periods=1).min(); high_max = df['high'].rolling(window=period, min_periods=1).max()
            df['stochastic_k'] = 100 * ((df['close'] - low_min) / (high_max - low_min).replace(0, 1))
            df['stochastic_d'] = df['stochastic_k'].rolling(window=3, min_periods=1).mean()
            df['stochastic_k'] = df['stochastic_k'].fillna(50); df['stochastic_d'] = df['stochastic_d'].fillna(50)
            return df
        except Exception as e: logger.error(f"Error Stochastic: {e}"); raise
    def _calculate_supertrend(self, df: pd.DataFrame, length=10, multiplier=3.0) -> pd.DataFrame: supertrend = ta.supertrend(df['high'], df['low'], df['close'], length=length, multiplier=multiplier); return df.join(supertrend)
    def _calculate_ichimoku(self, df: pd.DataFrame) -> pd.DataFrame:
        ichimoku = ta.ichimoku(df['high'], df['low'], df['close'])
        ichimoku_df = pd.concat(ichimoku, axis=1) if isinstance(ichimoku, tuple) else ichimoku
        return df.join(ichimoku_df)
    def _calculate_mfi(self, df: pd.DataFrame, length=14) -> pd.DataFrame: df[f'MFI_{length}'] = ta.mfi(df['high'], df['low'], df['close'], df['volume'], length=length); return df
    def _calculate_chaikin_oscillator(self, df: pd.DataFrame, fast=3, slow=10) -> pd.DataFrame:
        high, low, close, volume = df['high'], df['low'], df['close'], df['volume']; ad = ((2*close-high-low)/(high-low+1e-9))*volume; ad=ad.fillna(0); ad_cumsum=ad.cumsum()
        chaikin = ad_cumsum.ewm(span=fast, adjust=False).mean() - ad_cumsum.ewm(span=slow, adjust=False).mean(); df['Chaikin_Osc'] = chaikin; return df
    def _calculate_keltner(self, df: pd.DataFrame, length=20, scalar=1.5) -> pd.DataFrame: keltner = ta.kc(df['high'], df['low'], df['close'], length=length, scalar=scalar); return df.join(keltner)
    def _calculate_donchian(self, df: pd.DataFrame, lower_length=20, upper_length=20) -> pd.DataFrame: donchian = ta.donchian(df['high'], df['low'], lower_length=lower_length, upper_length=upper_length); return df.join(donchian)
    def _calculate_cci(self, df: pd.DataFrame, length=20) -> pd.DataFrame: df[f'CCI_{length}'] = ta.cci(df['high'], df['low'], df['close'], length=length); return df
    def calculate_bollinger_bands(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period = self.params['bollinger_period']; std_dev = self.params['bollinger_std']
            if len(df) > 10000 and self.params['enable_parallel']:
                with ThreadPoolExecutor(max_workers=2) as executor:
                    sma_future = executor.submit(lambda: df['close'].rolling(window=period, min_periods=1).mean()); std_future = executor.submit(lambda: df['close'].rolling(window=period, min_periods=1).std())
                    sma = sma_future.result(); rolling_std = std_future.result()
            else: sma = df['close'].rolling(window=period, min_periods=1).mean(); rolling_std = df['close'].rolling(window=period, min_periods=1).std()
            df['bollinger_upper'] = sma + (std_dev * rolling_std); df['bollinger_lower'] = sma - (std_dev * rolling_std); df['bollinger_mid'] = sma
            df['bollinger_bandwidth'] = (df['bollinger_upper'] - df['bollinger_lower']) / df['bollinger_mid'].replace(0, np.nan) # Avoid div by zero
            df['bollinger_pctb'] = (df['close'] - df['bollinger_lower']) / ((df['bollinger_upper'] - df['bollinger_lower']).replace(0, np.nan))
            return df
        except Exception as e: logger.error(f"Error Bollinger: {e}"); raise
    def calculate_atr(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            period = self.params['atr_period']; high_low = df['high'] - df['low']; high_close = (df['high'] - df['close'].shift()).abs(); low_close = (df['low'] - df['close'].shift()).abs()
            true_range = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1); df['atr'] = true_range.ewm(span=period, adjust=False).mean()
            return df
        except Exception as e: logger.error(f"Error ATR: {e}"); raise
    def calculate_anchored_vwap(self, df: pd.DataFrame, anchor_index: int) -> pd.Series:
        if not 0 <= anchor_index < len(df): raise ValueError("Anchor index out of bounds")
        price_volume = (df['close'] * df['volume']).copy(); price_volume[:anchor_index] = 0
        volume_cumsum = df['volume'].copy(); volume_cumsum[:anchor_index] = 0
        return (price_volume.cumsum() / volume_cumsum.cumsum().replace(0, np.nan)).ffill()
    ...
    def calculate_all_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            # Validate first, ensuring DatetimeIndex exists
            self.validate_dataframe(df) # This now checks for DatetimeIndex
            df_out = df.copy()
            logger.info("Starting indicator calculations...")
            ...
            df_out = (df_out
                .pipe(self.calculate_trend_indicators)
                .pipe(self.calculate_macd)
                .pipe(self.calculate_rsi)
                .pipe(self.calculate_stochastic)
                .pipe(self.calculate_bollinger_bands)
                .pipe(self.calculate_atr)
                .pipe(self.calculate_volume_indicators) # Corrected VWAP logic inside
                .pipe(self.calculate_dmi_adx)
                .pipe(self.calculate_vwma)
                .pipe(self.calculate_volume_ma)
                .pipe(self._calculate_supertrend)
            ...
            calculated_cols = set(df_out.columns) - set(df.columns)
            logger.info(f"Successfully calculated {len(calculated_cols)} indicators.")
            return df_out
        except Exception as e: logger.error(f"Overall indicator calc failed: {e}", exc_info=True); raise

# --- Standalone Script Execution Logic (Keep as provided by user) ---
# In main function
def main(input_file: Path, output_file: Path) -> bool:
    try:
        logger.info(f"Loading data from {input_file}")
        ...
        # Ensure DatetimeIndex and localize to Asia/Kolkata if naive
        if not isinstance(df.index, pd.DatetimeIndex):
            logger.warning("Index not datetime, attempting conversion...")
            df.index = pd.to_datetime(df.index)
        ...
        df_with_indicators.to_csv(output_file, compression=compression)
        logger.info(f"Results saved to {output_file}")
        return True
    except FileNotFoundError:
        logger.error(f"Input file not found: {input_file}")
        ...
        logger.error("Input file empty/corrupt")
        return False
    except Exception as e:
        logger.error(f"Unexpected error: {e}", exc_info=True)
        return False

# In IndicatorCalculator class
def validate_dataframe(self, df: pd.DataFrame) -> None:
    """Validate input dataframe structure and data integrity"""
    required_columns = ['open', 'high', 'low', 'close', 'volume']
    # Allow datetime column as fallback if index isn't DatetimeIndex
    if not isinstance(df.index, pd.DatetimeIndex):
        if 'datetime' in df.columns:
            logger.warning("Converting 'datetime' column to DatetimeIndex")
            df.index = pd.to_datetime(df['datetime'])
            if df.index.tz is None:
                df.index = df.index.tz_localize('Asia/Kolkata', ambiguous='infer')
        else:
            raise ValueError("DataFrame index must be a DatetimeIndex or contain a 'datetime' column.")
    missing = [col for col in required_columns if col not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns: {missing}")
    ...
    if df[required_columns].isna().any().any():
        logger.warning("NaNs found in OHLCV data after coerce.")
    min_period = max(1, max([
        *self.params['sma_periods'], *self.params['ema_periods'],
        self.params['rsi_period'], self.params['stochastic_period'],
        self.params['bollinger_period'], self.params['atr_period'],
        self.params['dmi_length'], self.params['adx_smoothing'],
        self.params['vol_ma_len'],
        self.params['macd_params'][1]
    ]))
    if len(df) < min_period:
        logger.warning(f"Data length ({len(df)}) less than max period ({min_period}).")
    if (df['high'] < df['low']).any():
        raise ValueError("High price less than low price found.")
    if ((df['close'] > df['high']) | (df['close'] < df['low'])).any():
        raise ValueError("Close price outside high/low range found.")
    if (df['volume'] < 0).any():
        raise ValueError("Negative volume found.")
    logger.debug("DataFrame validation passed.")
    
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Calculate technical indicators', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--input', required=True, type=Path, help='Input CSV (datetime index, ohlcv columns)')
...
if __name__ == "__main__":
    try:
        args = parse_args()
        params = {'parallel_workers': args.workers, 'enable_parallel': not args.no_parallel}
        # Note: The Calculator instance isn't using the parsed params here, using defaults
        # Modify main or Calculator.__init__ if params need passing from CLI
        success = main(args.input, args.output)
        sys.exit(0 if success else 1)
    except Exception as e: logger.error(f"Fatal error: {e}", exc_info=True); sys.exit(1)