...
# --- Define Enhanced AlphaTrendStrategy Class ---
class AlphaTrendStrategy:
    """
    Implements the AlphaTrend trading strategy with ENHANCED parameters
    aiming for more sensitivity and potentially more trades.
    ...
    def __init__(
        ...
        coeff: float = 0.6,
        # --- Enhanced Parameters ---
        ap: int = 14,             # Changed from 10 to 14 (more standard)
        macd_fast: int = 12,
        macd_slow: int = 26,
        macd_signal: int = 9,
        supertrend_period: int = 10,
        supertrend_multiplier: float = 2.5, # Changed from 3.0 (more reactive)
        rsi_buy_lower: int = 35,            # Kept relaxed
        rsi_buy_upper: int = 85,            # Kept relaxed
        rsi_sell_lower: int = 25,           # Kept relaxed
        rsi_sell_upper: int = 65,           # Kept relaxed
        rsi_extreme_long: int = 80,         # Kept relaxed
        rsi_extreme_short: int = 20,        # Kept relaxed
        rsi_tighten_long: int = 70,         # Kept relaxed
        rsi_tighten_short: int = 30,        # Kept relaxed
        volatility_threshold: float = 0.25, # Kept relaxed
        entry_condition_count: int = 3,     # Kept relaxed
        atr_stop_mult_early: float = 2.5,
        atr_stop_mult_late: float = 3.5,
        atr_stop_mult_tight: float = 2.25   # Changed from 2.0 (slightly less tight)
    ):
        """
        Initializes the strategy with enhanced parameters.
        """
        # Store parameters
        self.coeff = coeff
        self.ap = ap
        ...
        strategy_logger.info(
            f"Init ENHANCED AlphaTrend: coeff={coeff}, ap={ap}, "
            f"macd=({macd_fast},{macd_slow},{macd_signal}), "
            f"supertrend=({supertrend_period},{supertrend_multiplier}), " # Enhanced
            f"RSI ranges Buy=({rsi_buy_lower}-{rsi_buy_upper}), Sell=({rsi_sell_lower}-{rsi_sell_upper}), "
            f"Volatility Threshold={volatility_threshold}, Conditions Needed={entry_condition_count}, "
            f"Stop Multipliers Early={atr_stop_mult_early}, Late={atr_stop_mult_late}, Tight={atr_stop_mult_tight}, " # Enhanced
            f"RSI Extremes=({rsi_extreme_short}/{rsi_extreme_long}), RSI Tighten=({rsi_tighten_short}/{rsi_tighten_long})"
        )
    ...
    # --- generate_signals Method (Uses updated parameters from __init__) ---
    def generate_signals(self, data: pd.DataFrame, prev_close: Optional[float] = None) -> pd.DataFrame:
        """
        Generates trading signals and positions based on the Enhanced AlphaTrend Logic.
        """
        required_output_cols = ['signal', 'position', 'exit_reason', 'buy_conditions_met', 'sell_conditions_met']
        ...
        for col in required_output_cols:
             ...
             else: empty_df_output[col] = empty_df_output[col].astype(float)

        if data.empty:
            strategy_logger.warning("Enhanced AlphaTrend: empty input dataframe.")
            return empty_df_output

        df = data.copy()

        # --- 1. Ensure DatetimeIndex and Timezone ---
        # (Keep the robust DatetimeIndex/Timezone handling from previous version)
        ...
        # ATR (Uses self.ap - now 14 by default)
        atr_strategy_col = f'atr_sma_{self.ap}'
        atr_sim_col = 'atr_14'; atr_sim_period = 14 # Keep calculating ATR 14 for simulation compatibility
        try:
            ...
            # Calculate ATR 14 only if different from strategy ap
            if self.ap != atr_sim_period:
                df[atr_sim_col] = ta.sma(df['tr'], length=atr_sim_period); df[atr_sim_col] = df[atr_sim_col].bfill()
            else:
                 df[atr_sim_col] = df[atr_strategy_col] # If ap is 14, just copy it
        ...
        # RSI and Slope (Uses self.ap - now 14 by default)
        ...
        # MACD (Uses standard params unless overridden in __init__)
        ...
        # Supertrend (Uses self.supertrend_period, self.supertrend_multiplier)
        ...
        # --- 4. Calculate Entry Conditions (Using Enhanced Parameters) ---
        ...
        df['buy_cond2'] = (df['close'] - df['close_shift2']).abs() > self.volatility_threshold # Enhanced
        df['buy_cond3'] = (df[rsi_col] > self.rsi_buy_lower) & (df[rsi_col] < self.rsi_buy_upper) # Enhanced
        ...
        df['sell_cond2'] = (df['close'] - df['close_shift2']).abs() > self.volatility_threshold # Enhanced
        df['sell_cond3'] = (df[rsi_col] > self.rsi_sell_lower) & (df[rsi_col] < self.rsi_sell_upper) # Enhanced
        ...
        # Final Entry Signals (Uses self.entry_condition_count - Enhanced)
        ...
        for i, idx in enumerate(df.index):
            ...
            if position_active == 1:
                ...
                elif current_rsi > self.rsi_extreme_long: exit_reason_str = f'RSI Extreme (> {self.rsi_extreme_long})'; exit_triggered = True # Enhanced threshold
                elif current_rsi < self.rsi_extreme_short: exit_reason_str = f'RSI Extreme (< {self.rsi_extreme_short})'; exit_triggered = True # Enhanced threshold
            ...
            if pd.notna(current_atr) and current_atr > 0 and not exit_triggered and current_time >= time_0930:
                if position_active == 1 and not morning_trade_active:
                    atr_mult = self.atr_stop_mult_tight if current_rsi > self.rsi_tighten_long else self.atr_stop_mult_late # Enhanced threshold
                    ...
                    atr_mult = self.atr_stop_mult_tight if current_rsi < self.rsi_tighten_short else self.atr_stop_mult_late # Enhanced threshold
                    potential_new_stop = current_close + (atr_mult * current_atr)
        ...
        # Ensure atr_strategy_col uses the correct self.ap value
        final_indicator_cols = ['alphatrend', f'atr_sma_{self.ap}', atr_sim_col, rsi_col, macd_final_col, macds_final_col, 'rsi_slope',
