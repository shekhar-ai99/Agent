...
#     ORB_START_TIME = datetime.time(9, 15) # Hardcoded
#     ORB_DURATION_MINUTES = 15           # Hardcoded
...
def create_opening_range_breakout_strategy(
    ...
    entry_buffer_pct: float = 0.0005, # Example: making buffer tunable
    **kwargs
):
    strategy_name = f"ORB_{orb_start_hour:02d}{orb_start_minute:02d}_{orb_duration_minutes}min"
    # Make ORB_START_TIME from tunable params
    ORB_ACTUAL_START_TIME = dt_time(orb_start_hour, orb_start_minute)
    # Use orb_duration_minutes from params

    def strategy(row: pd.Series, data_history: pd.DataFrame = None) -> str:
        current_time_dt = row.name # This is already a datetime object from simulation_engine

        # Use ORB_ACTUAL_START_TIME and orb_duration_minutes from outer scope
        orb_window_start_dt = current_time_dt.replace(hour=ORB_ACTUAL_START_TIME.hour, 
                                                      minute=ORB_ACTUAL_START_TIME.minute, 
                                                      second=0, microsecond=0)
        orb_window_end_dt = orb_window_start_dt + timedelta(minutes=orb_duration_minutes)

        if current_time_dt <= orb_window_end_dt:
            return "hold" # Still in ORB formation or before it starts effectively for this bar

        # Filter for ORB data only from the current day up to orb_window_end_dt
        day_start_dt = current_time_dt.replace(hour=0, minute=0, second=0, microsecond=0)

        # Ensure data_history is for the current day and within the ORB window
        # This requires data_history to be the full day's data ideally, or at least cover the ORB
        orb_data_mask = (data_history.index >= orb_window_start_dt) & \
                        (data_history.index <= orb_window_end_dt) & \
                        (data_history.index.date == current_time_dt.date()) # Ensure same day

        orb_data = data_history.loc[orb_data_mask]

        if orb_data.empty:
            # logger.debug(f"[{strategy_name} @ {current_time_dt}] ORB data empty for window {orb_window_start_dt} to {orb_window_end_dt}")
            return "hold"

        orb_high = orb_data["high"].max()
        orb_low = orb_data["low"].min()

        # ENTRY_BUFFER = orb_high * entry_buffer_pct # Example of dynamic buffer
        # STOP_LOSS_BUFFER = orb_low * entry_buffer_pct

        if pd.isna(orb_high) or pd.isna(orb_low): return "hold"

        price = row["close"]
        # Buffer should be absolute or based on range, not just fixed value
        # For simplicity, keeping your buffer logic for now
        buffer_val = (orb_high - orb_low) * 0.05 # Example: 5% of range as buffer
                                                # Or use a fixed point buffer, or ATR based

        if price > orb_high + buffer_val: # Use a calculated buffer
            return "buy_potential"
        elif price < orb_low - buffer_val: # Use a calculated buffer
